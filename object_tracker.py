...
def compute_color(label: str) -> tuple:
    """
...
def draw_boxes(img: np.array, bbox: np.array, object_id: np.array,
                identities: np.array, txt_path: str, frame_num: int) -> None:
    ...
    for i, box in enumerate(bbox):
        ...
        with open(txt_path + '.txt', 'a') as f:
            f.write(f'{frame_num},{id},{str(names[object_id[i]])},{x1},{y1},{w},{h},\n')
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                #     s = f'{s} {n} {names[int(c)]}' # add to string
                ...
                if len(outputs) > 0:
                    ...
                    draw_boxes(im0, bbox_xyxy, object_id, identities, txt_path, frame_num)
